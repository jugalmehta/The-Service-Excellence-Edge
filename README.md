...
industries.html       Industries (6 industries)
case-studies.html     Case Studies (3 full write-ups)
...
script.js             Shared behaviour (mobile nav, reveal animation, contact form)
...
- **Fonts**: the site loads Fraunces and Inter from Google Fonts via a
  `<link>` tag in each page's `<head>`. No local font files are needed,
  but an internet connection is required for fonts to load — this is
  standard for static sites and works on any host.
...
footer, update it in all seven `.html` files.
